...
MM.widgets = {
    ...
    dialog: function(text, options) {
        ...
        if (options.width) {
            MM.widgets.dialogSize(options.width);
        } else {
            MM.widgets.dialogSize();
        }
        ...
        // On orientation change resize the dialog if it is open.
        $(window).bind('orientationchange', MM.widgets.dialogSize);
    ...
    dialogSize: function(setWidth) {
        var dialogDiv = "#app-dialog > div";

        // If this is a phone, we make the dialogs fill the screen.
        if (matchMedia('only screen and (max-width : 320px) and (orientation : portrait)').matches
            || matchMedia('only screen and (max-width : 640px) and (orientation : landscape)').matches) {
                ...
                if (setWidth) {
                    $(dialogDiv).css('width', setWidth);
                } else {
                    $(dialogDiv).css('width', window.innerWidth + "px");
                }

                $(dialogDiv).css('height', window.innerHeight + "px");
                $(dialogDiv + " .modalContent").css('height', ($(dialogDiv).innerHeight() - $(dialogDiv + " .modalHeader").innerHeight() - $(dialogDiv + " .modalFooter").innerHeight()) + "px");
            } else if (setWidth) {
                $(dialogDiv).css('width', setWidth);
            } else {
                var newWidth = $(document).innerWidth() / 2;
                $(dialogDiv).css('width', newWidth);
            }
    },
    ...
    dialogSetTop: function() {
        ...
        if (matchMedia('only screen and (max-width : 320px) and (orientation : portrait)').matches
            ...
            $("#app-dialog > div").css('margin-top', (viewPortHeight / 2) - (dialogHeight / 2) + "px");
        }
    },
    ...
    dialogClose: function() {
        ...
        $(window).unbind('orientationchange', MM.widgets.dialogSize);
