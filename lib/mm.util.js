...
MM.util = {
    ...
    isTouchDevice: function() {
        // No computers are touch devices... ever.
        if (MM.inComputer) {
            return false;
        }
    ...
    setPanelsScreenHeight: function() {
        ...
        $('#add-site, #main-wrapper').css('height', $(document).innerHeight());

        var headerHeight = $('header.header-main').height();
        $('#panel-center, #panel-right, #panel-left').css(
            'height', $(document).innerHeight() - headerHeight
        );
    },
    ...
    applyOverflow: function(position) {
        $('#panel-' + position).css({'overflow-y':'scroll', 'overflow-x':'hidden'});
    ...
    helpMeLogin: function () {
        MM.widgets.dialog(MM.lang.s("helpmelogin"), {title: MM.lang.s("help")});
    ...
    /**
     * Sets up accordions where necessary. Needs to be called after a page
     * has rendered.
     */
    setupAccordion: function(parent) {
        if (parent === undefined || parent === null) {
            return;
        }
        $(parent).find('.submenu').hide();
        $(parent).find('.toogler').bind(MM.clickType, function(e) {
        ...
        $(parent).find('.close.toogler').on(MM.clickType, function(e){
            e.preventDefault();
            $(this).parent('.toogler').next().slideToggle(300);
            $(this).parent('.toogler').toggleClass("collapse expand");
        });
