...
var MM = {
    ...
    updateMenuItemsIndicators: function() {
        MM.moodleWSCall(
            'core_webservice_get_item_counts',
            {userid: MM.site.get('userid')},
            function(res) {
                var updatables = MM.getConfig('menu_items_indicators_updatable');
                _.each(updatables, function(updatable) {
                    var $totalEl = $("#menu-items-indicator-" + updatable.pluginname);
                    var $newEl = $("#menu-items-new-" + updatable.pluginname);
                    var $topEl = $("#top-menu-items-new-" + updatable.pluginname);
                    var prevTotal = parseInt($totalEl.text());
                    var prevNew = parseInt($newEl.text());
                    var total = parseInt(res[updatable.wsname]);
                    var new_ = prevNew;
                    if (total > prevTotal) { 
                        new_ = new_ + total - prevTotal; 
                    }
                    $totalEl.text(total);
                    $newEl.text(new_);
                    if ($topEl) {
    ...
    resetMenuItemsIndicator: function(pluginname) {
        $("#menu-items-new-" + updatable.pluginname).text('0');
        $("#top-menu-items-new-" + updatable.pluginname).text('0');
    },
    ...
    setUpNavigation: function() {
        $(document).on('siteLoaded', function() {
            MM.siteLoaded = true;
            if (MM.siteLoaded === true && MM.menuPluginLoaded === true && MM.homepagePluginLoaded === true) {
                MM._setUpHomepage();
                MM._setUpNavigation();
            }
        });
        $(document).on('pluginLoaded', function(event, plugin) {
            if (plugin.settings.type == 'menu') {
                MM.menuPluginLoaded = true;
            }
            if (plugin.settings.name == MM.getConfig('homepage')) {
    ...
    getDeviceOrientation: function(e) {
        switch(window.orientation)
        {
          ...
          // alert('Landscape horay This SCORM should work now :0)');
            break;
          default:
           this.deviceOrientation = 'portrait';
           // alert('Portrait Ohhh! This SCORM only works in landscape');
            break;
        }
...
*/
    ...
    requireMainSiteLogin: function(callback) {
        var url = MM.config.current_site.siteurl + "/login/index.php";
    ...
    processMainSiteLogin: function(siteurl, username, password, successCallback, failureCallback) {
        var url = siteurl + "/login/index.php";
        var data = {
            username: username,
            password: password,
            jsonresponse: 1
