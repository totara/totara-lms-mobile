...
var MM = {
    ...
    loadSettings: function() {
        ...
        var siteSpecific = MM.db.getSiteSpecificConfig();
        var globalSettings = MM.db.getGlobalConfig();

        _.each(siteSpecific, function(setting) {
            MM.config[setting.get('name')] = setting.get('value');
        });

        _.each(globalSettings, function(setting) {
            MM.config[setting.get('name')] = setting.get('value');
        });
    },
    ...
    setConfig: function(name, value, site) {
        ...
        MM.db.insert('settings', setting, !site);
    },
    ...
    orientationChangeHandler: function(e) {
        ...
        $("#panel-left").css(
            'height',
            parseInt($(document).innerHeight(), 10) - $(".header-main").height()
        );
    ...
    loadSite: function(siteId) {
        MM.log('Loading site');
        MM.site = MM.db.get('sites', siteId, true);
        MM.sync.init();
        MM.setUpConfig();
        MM.setUpLanguages();
        MM.setUpNavigation();
    ...
    setUpNavigation: function() {
        ...
        var output = MM.tpl.render($("#top_bar_template").html(), {'menu_items':values});
    ...
    loadCourses: function(courses) {
        ...
        if (MM.deviceType == 'tablet') {
            ...
            //MM.panels.menuShow(false, {animate:false});
            MM.panels.hide('right', false);
        }
    },
            ...
            success:function(json) {
                if (typeof(json.token) != 'undefined') {
                    ...
                    MM.moodleWSCall('moodle_webservice_get_siteinfo', {}, function(d) {
                        ...
                        d.token = mytoken;
                        var site = MM.db.insert('sites', d, true);
                        MM.setConfig('current_site', d, false);
                        MM.loadSite(site.id);
