...
var MM = {
    ...
    setupOrientationDetection: function() {
        window.addEventListener('orientationchange', MM.handleOrientationChange);
        MM.handleOrientationChange(false);
    },

    handleOrientationChange: function(trigger) {
        trigger = typeof(trigger) === 'undefined' ? true : trigger;
        var prev = MM.orientation;
        var orientation = 'portrait';
        if (window.orientation === -90 || window.orientation === 90) {
            orientation = 'landscape';
        }
        if (trigger) {
            $(document).trigger('orientationchange', orientation);
        }
