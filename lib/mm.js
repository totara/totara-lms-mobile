...
var MM = {
    ...
    setUpNavigation: function() {
        $(document).on('siteLoaded', function() {
            MM.siteLoaded = true;
            if (MM.siteLoaded === true && MM.menuPluginLoaded === true) {
                MM._setUpNavigation();
            }
        });
...
console.log("Loading settings");
    ...
    loadLayout: function() {
            ...
            $(".remember-option").on('click', function() {
                ...
                if ($(this).data('option') === 1) {
                    ...
                    if (MM._siteExists(url, username) === true &&
                        MM.get('sites', hex_md5(url + username)).get('token') !== undefined
                    ) {
        ...
        // Dom is ready!. Moved from the top of this function because it's only
        // at the end, when everything is loaded, that the Router should come
        // into play.
    ...
    loadSite: function(siteId) {
        ...
        var data = {}; 
        var callback = MM.homepageCallback;
     ...
     * Homepage callback
     */
    homepageCallback: function(response) {
        var html = '<div class="panel-content home"><div class="sections has-user-content">' + response + '</div><button class="button view-menu">' + MM.lang.s("home-viewmenu-show") + '</button><a href="#my-courses" class="button view-courses">' + MM.lang.s("home-viewcourses") + '</a></div>';
        MM.panels.html("center", html);

        $(".view-menu").on("click", function() {
            if (MM.navigation.visible == false) {
                MM.navigation.show();
...
console.log(thrownError);
    ...
    handleMainSiteLoginForm: function(ev) {
        ...
        var data = {
            ...
            password: $(this).find("#main-site-login-form-password").val(),
            jsonresponse: 1
        }
        $.post(url, data, function(response) {
            if (response.success) {
